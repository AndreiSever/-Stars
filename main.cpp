...
void
RotatePoint(float *x, float *y, float rotation) /// Поворот
{
  float tmpX, tmpY;
...
void
MoveStars(void) /// Перемещение звезд
{
  float offset;
  GLint n;

  offset = speed * 60.0+ nitro;
...
StarPoint(GLint n)
{
  ...
  if (x0 >= 0.0 && x0 < windW && y0 >= 0.0 && y0 < windH) { /// Проверяем точки,чтобы они находились в экране
    return GL_TRUE;
  } else {
    return GL_FALSE;
  }
...
Init(void)
{
  ...
  srand((unsigned int) time(NULL));
    ///Количество звезд не превышает пределы,то вызываем функцию создания новой звездыау
  ...
  glDisable(GL_DITHER); /// отключение прорисовки
}

void  /// Настраивает изменение размера окна
...
Visible(int state)
{
  if (state == GLUT_VISIBLE) {
    glutIdleFunc(Idle); /// Подключаем функции движения и обновления звезд
  } else {
    glutIdleFunc(NULL);
  }
}
...
///По циклу проверяем конец строк, и отключаем двойную буферизацию
  for (i = 1; i < argc; i++) {
...
{
  GLenum type;
/// Прописываем различные настройки
